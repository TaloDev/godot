...
##
## This API is used to fetch info about group memberships. Player groups are used to segment players
## into groups based on filter criteria.
##
## @tutorial: https://docs.trytalo.com/docs/godot/groups


## Get the members of a group. If the group is public (as set in the dashboard), the members will be
## returned. If the group is private, only a membership count will be returned. Members are
## paginated and a page number can be specified.
