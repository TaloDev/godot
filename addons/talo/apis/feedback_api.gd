...
func get_categories() -> Array[TaloFeedbackCategory]:
	...
	match res.status:
		200:
			...
			categories.assign(
				res.body.feedbackCategories.map(
					func(category: Dictionary):
						return TaloFeedbackCategory.new(category),
				)
			)
			return categories
		_:
...
func send(
	...
	var res := await client.make_request(
		...
		"/categories/%s" % category_internal_name,
		{ comment = comment, props = props_to_send },
	)
