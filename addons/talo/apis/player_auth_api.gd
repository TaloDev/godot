...
func register(
	...
	if verification_enabled and email.is_empty():
		...
		var error := TaloPlayerAuthError.from_response(
...
func login(identifier: String, password: String) -> PlayerAuthLoginResult:
	var res := await client.make_request(
		HTTPClient.METHOD_POST,
		"/login",
		{ identifier = identifier, password = password, withRefresh = true },
	)
...
func change_identifier(current_password: String, new_identifier: String) -> PlayerAuthResult:
	var res := await client.make_request(
		HTTPClient.METHOD_POST,
		"/change_identifier",
		{ currentPassword = current_password, newIdentifier = new_identifier },
	)
...
func migrate_account(
	...
	var res := await client.make_request(
		HTTPClient.METHOD_POST,
		"/migrate",
		{ currentPassword = current_password, service = new_service, identifier = new_identifier },
	)
