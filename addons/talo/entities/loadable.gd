...
func _convert_serialised_data(item: Dictionary) -> Variant:
	match Talo.saves.get_format_version():
		"godot.v1":
			return type_convert(item.value, int(item.type))
		_:
			return str_to_var(item.value)
...
func get_latest_data() -> Array[Dictionary]:
	...
	data.assign(
		_saved_fields.keys().map(
			func(key: String):
				var value = _saved_fields[key]
				return { key = key, value = var_to_str(value), type = str(typeof(value)) },
		)
	)
	return data
