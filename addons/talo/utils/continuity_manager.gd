	...
	_requests.push_back(
		{
			...
			headers = headers.filter(
				func(h: String):
					return h.find("Authorization") == -1 and h.find("X-Talo-Signature") == -1,
			),
			timestamp = timestamp,
		}
	)
...
func _on_timeout():
	...
	for i in range(10):
		...
		if not req.headers.any(
			func(h: String):
				return h.find(_CONTINUITY_TIMESTAMP_HEADER) != -1,
		):
...
func request_can_be_replayed(
	method: HTTPClient.Method,
	url: String,
	res: TaloClient.TaloClientResponse,
) -> bool:
	if not Talo.settings.continuity_enabled:
		return false

	if _EXCLUDED_ENDPOINTS.any(
		func(endpoint: String):
			return url.find(endpoint) != -1,
	):
		return false
