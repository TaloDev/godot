...
func read_offline_saves() -> Array[TaloGameSave]:
	...
	var data_arr :Variant = JSON.parse_string(content.get_as_text())
	assert(data_arr is Array)

	var res: Array[TaloGameSave] = []
	res.assign(data_arr.map(func (data: Dictionary) -> TaloGameSave: return TaloGameSave.new(data)))

	return res

func write_offline_saves(offline_saves: Array[TaloGameSave]) -> void:
	var saves := FileAccess.open_encrypted_with_pass(_offline_saves_path, FileAccess.WRITE, Talo.crypto_manager.get_key())
	saves.store_line(JSON.stringify(offline_saves.map(func (save: TaloGameSave) -> Dictionary: return save.to_dictionary())))
...
func delete_offline_save(save: TaloGameSave) -> void:
	var offline_saves: Array[TaloGameSave] = read_offline_saves()
	offline_saves = offline_saves.filter(func (s: TaloGameSave) -> bool: return s.id != save.id)
...
func get_synced_saves(online_saves: Array[TaloGameSave]) -> Array[TaloGameSave]:
	...
	if not offline_saves.is_empty():
		for online_save in online_saves:
			var filtered := offline_saves.filter(func (save: TaloGameSave) -> bool: return save.id == online_save.id)
...
func register_fields_for_saved_objects() -> void:
...
func get_save_content() -> Dictionary:
	return {
		objects = _registered_saved_objects.map(func (saved_object: TaloSavedObject) -> Dictionary: return saved_object.to_dictionary())
	}

func replace_save(new_save: TaloGameSave) -> void:
	var existing_saves := all_saves.filter(func (save: TaloGameSave) -> bool: return save.id == new_save.id)
...
func get_latest_save() -> TaloGameSave:
	...
	dupe.sort_custom(
		func (a: TaloGameSave, b: TaloGameSave) -> bool:
