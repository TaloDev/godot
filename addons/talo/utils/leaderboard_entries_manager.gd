...
func get_entries(internal_name: String) -> Array[TaloLeaderboardEntry]:
	return _current_entries.get(
		internal_name,
		Array([], TYPE_OBJECT, "RefCounted", TaloLeaderboardEntry),
	)
...
func upsert_entry(
	...
	entry_to_upsert: TaloLeaderboardEntry,
	bump_positions: bool = false,
) -> void:
	...
	named_entries = _current_entries.get_or_add(internal_name, named_entries).filter(
		...
		func(e: TaloLeaderboardEntry) -> bool:
			return e.id != entry_to_upsert.id,
	)
	...
	if bump_positions:
		...
		var collision_index := named_entries.find_custom(
			func(e: TaloLeaderboardEntry) -> bool:
				return e.id != entry_to_upsert.id and e.position == entry_to_upsert.position,
		)
