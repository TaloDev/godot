...
func _on_pressed() -> void:
	...
	var result: PlayersAPI.PlayerUpdateResult = await Talo.current_player.set_prop(
		prop_name,
		prop_value,
	)
	if not result.rejected_props.is_empty():
		var reasons := result.rejected_props.map(
			func(rp: TaloRejectedProp):
				return "[%s] %s" % [rp.key, rp.message],
		)
