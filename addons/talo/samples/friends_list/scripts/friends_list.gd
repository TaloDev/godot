...
const SubscriptionsManager := preload(
	"res://addons/talo/samples/friends_list/scripts/subscriptions_manager.gd"
)
...
func _connect_signals() -> void:
	...
	Talo.player_relationships.relationship_request_received.connect(
		_on_relationship_request_received
	)
	Talo.player_relationships.relationship_request_cancelled.connect(
		_on_relationship_request_cancelled
	)
...
func _send_intro_feed_messages() -> void:
	...
	_ui_manager.add_feed_message(
		"Go to Debug > Customize Run Instances and tick 'Enable Multiple Instances'.",
		Color.GRAY,
	)
...
func _on_pending_requests_updated() -> void:
	_ui_manager.refresh_pending_requests_list(
		_subscriptions_manager.get_pending_requests(),
		_on_approve_request,
	)
	_on_players_updated()
...
func _on_outgoing_requests_updated() -> void:
	_ui_manager.refresh_outgoing_requests_list(
		_subscriptions_manager.get_outgoing_requests(),
		_on_cancel_request,
	)
	_on_players_updated()
...
func _on_presence_changed(
	...
	var identifier := (
		"You are"
		if presence.player_alias.id == Talo.current_alias.id
		else (presence.player_alias.identifier + " is")
	)
...
func _on_relationship_request_received(player_alias: TaloPlayerAlias) -> void:
	_ui_manager.add_feed_message(
		"%s sent you a friend request!" % player_alias.identifier,
		Color.YELLOW,
	)
...
func _on_relationship_confirmed(player_alias: TaloPlayerAlias) -> void:
	_ui_manager.add_feed_message(
		"%s accepted your friend request!" % player_alias.identifier,
		Color.GREEN,
	)
...
func _on_relationship_ended(player_alias: TaloPlayerAlias) -> void:
	_ui_manager.add_feed_message(
		"You are no longer friends with %s" % player_alias.identifier,
		Color.GRAY,
	)
	await _subscriptions_manager.load_friends()
	await _subscriptions_manager.load_pending_requests()
...
func _on_approve_request(requester: TaloPlayerAlias) -> void:
	_ui_manager.add_feed_message(
		"Accepting friend request from %s..." % requester.identifier,
		Color.CYAN,
	)
	...
	if success:
		_ui_manager.add_feed_message(
			"Accepted! %s is now your friend" % requester.identifier,
			Color.GREEN,
		)
		await _refresh_all_data()
	else:
...
func _on_cancel_request(player: TaloPlayerAlias) -> void:
	_ui_manager.add_feed_message(
		"Cancelling friend request to %s..." % player.identifier,
		Color.CYAN,
	)
	var success := await _subscriptions_manager.remove_friend(player)
	if success:
		_ui_manager.add_feed_message(
			"Cancelled friend request to %s" % player.identifier,
			Color.GRAY,
		)
		await _refresh_all_data()
	else:
		_ui_manager.add_feed_message(
			"Failed to cancel request to %s" % player.identifier,
			Color.RED,
		)
