...
func _init(
	feed: VBoxContainer,
	feed_scroll: ScrollContainer,
	players: VBoxContainer,
	subscriptions: VBoxContainer,
	pending: VBoxContainer,
	outgoing: VBoxContainer,
) -> void:
...
func refresh_players_list(
	...
	for alias in aliases:
		...
		var already_friends := subscriptions_manager.get_friends().find_custom(
			func(a):
				return a.id == alias.id,
		) != -1
		var has_outgoing_request := subscriptions_manager.get_outgoing_requests().find_custom(
			func(a):
				return a.id == alias.id,
		) != -1
		var has_pending_request := subscriptions_manager.get_pending_requests().find_custom(
			func(a):
				return a.id == alias.id,
		) != -1
		...
		if not already_friends and not has_outgoing_request:
			button.pressed.connect(
				func():
					add_friend_callback.call(alias),
			)
...
func refresh_friends_list(
	friends: Array[TaloPlayerAlias],
	remove_friend_callback: Callable,
) -> void:
	...
	for friend in friends:
		...
		button.pressed.connect(
			func():
				remove_friend_callback.call(friend),
		)
		container.add_child(button)
...
func refresh_pending_requests_list(
	pending_requests: Array[TaloPlayerAlias],
	approve_callback: Callable,
) -> void:
	...
	for request in pending_requests:
		...
		approve_button.pressed.connect(
			func():
				approve_callback.call(request),
		)
...
func refresh_outgoing_requests_list(
	outgoing_requests: Array[TaloPlayerAlias],
