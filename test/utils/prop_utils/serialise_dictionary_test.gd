...
func test_serialise_dictionary_with_array_key_expands_values() -> void:
	...
	assert_array(result).contains_exactly(
		[{ key = "items[]", value = "sword" }, { key = "items[]", value = "shield" }]
	)
...
func test_serialise_dictionary_with_mixed_scalar_and_array() -> void:
	var result := TaloPropUtils.serialise_dictionary(
		{ "color": "red", "items[]": ["sword", "shield"] }
	)

	assert_array(result).contains({ key = "color", value = "red" })
	assert_array(result).contains({ key = "items[]", value = "sword" })
	assert_array(result).contains({ key = "items[]", value = "shield" })
